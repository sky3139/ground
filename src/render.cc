...
//     pcl::PointCloud<pcl::PointXYZ> &pointCloud = *point_cloud_ptr; // 定义点云对象
...
//         pcl::visualization::RangeImageVisualizer::getRangeImageWidget(rangeImage, -std::numeric_limits<float>::infinity(), std::numeric_limits<float>::infinity(), false);
//     pcl::visualization::PCLVisualizer viewer("3D Viewer"); // 窗口名
//     viewer.addPointCloud(point_cloud_ptr, "original point cloud");
//     range_image_borders_widget->showRangeImage(rangeImage, -std::numeric_limits<float>::infinity(), std::numeric_limits<float>::infinity(), false);
...
// -----Parameters-----
// --------------------
float angular_resolution_x = 0.02f,
...
int main (int argc, char** argv)
{
  ...
  else
  {
    ...
    for (float x=-0.02f; x<=0.02f; x+=0.01f)
    {
      for (float y=-0.02f; y<=0.02f; y+=0.01f)
      {
  ...
  // -----------------------------------------------
  // -----1.Create RangeImage from the PointCloud by Spherical -----
  // -----tow pcl::createFromPointCloud include two function,there are different parameters(overload function)------------------------------------------
  ...
  //1度=π/180≈0.01745弧度，1弧度=180/π≈57.3度。 degree convert to radian
  ...
  //pcl::io::savePNGFile("rangeImage.png",point_cloud);
  //________________________________________________________________________________________

  // -----------------------------------------------
  // -----2.Create RangeImage from the PointCloud by plane-----
  // -----------------------------------------------

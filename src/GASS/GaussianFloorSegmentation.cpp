...
namespace pcl
{
    ...
    void GaussianFloorSegmentation<PointT>::sectorINSAC(int sector_index)
    {
        ...
        size_t num_points = sig_points.size() < static_cast<size_t>(this->params.num_seed_points_) ? sig_points.size() : static_cast<size_t>(this->params.num_seed_points_);
    ...
    template <typename PointT>
    std::vector<int> &GaussianFloorSegmentation<PointT>::getGD()
    {
        this->genPolarBinGrid();
        for (int i = 0; i < this->params.num_bins_a_; ++i)
        {

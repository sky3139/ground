...
void FloorSegment::Run(const pcl::PointCloud<pcl::PointXYZI>::Ptr cloudIn, std::vector<int> &lable)
{
	pcl::PointCloud<pcl::PointXYZI>::Ptr cloud(new pcl::PointCloud<pcl::PointXYZI>());
...
#include "../pcd_io.h"
void GPF_detection(std::string path, std::string filename, const config &cfg)
{
	// read cloud data.
